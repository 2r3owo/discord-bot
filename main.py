...
<button id="dotBtn" onclick="setTool('dot')">• 도트</button>
...
function setTool(t){
  tool=t;
  ['brush','neon','eraser','bucket','dot'].forEach(k=>$(''+k+'Btn').classList.toggle('active',k===t));
}
function pos(e){
...
function drawDot(px,py){
  ...
  x.beginPath();x.arc(px,py,Math.max(1,+$('size').value/2),0,Math.PI*2);x.fill();
}
...
function start(e){
  ...
  else if(tool==='dot'){drawDot(p.x,p.y);down=false;}
}
function move(e){
  if(!down)return;
  e.preventDefault();
