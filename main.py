...
let tool='brush',down=false,sx=0,sy=0,lx=0,ly=0,h=[],f=[];let pixelSize=12;
...
function pixel(p){let s=+size.value<4?4:pixelSize;let px=Math.floor(p.x/s)*s,py=Math.floor(p.y/s)*s;x.fillStyle=color.value;x.globalAlpha=+alpha.value/100;x.fillRect(px,py,s,s)}
function neonLine(a,b){x.save();x.globalCompositeOperation='lighter';x.shadowColor=color.value;x.shadowBlur=25;x.strokeStyle=color.value;x.lineWidth=+size.value;x.beginPath();x.moveTo(a.x,a.y);x.lineTo(b.x,b.y);x.stroke();x.shadowBlur=8;x.stroke();x.restore()}
