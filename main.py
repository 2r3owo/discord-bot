...
def download_song(search_or_url, guild_id):
    ...
    # Railway에서 특정 YouTube 클라이언트/JS 런타임 때문에 실패하지 않도록
    # 가장 일반적인 설정부터 순서대로 시도합니다.
    attempts = [
        {},
    ...
    for attempt in attempts:
        ...
        # 설치되지 않은 Deno/PO Token 설정이 다운로드를 막지 않도록 제거합니다.
        ...
        try:
            ...
            if not os.path.exists(filepath):
                ...
                if not candidates:
                    raise FileNotFoundError("YouTube 음원 파일이 생성되지 않았습니다.")
                filepath = str(candidates[0])

            print(f"✅ YouTube 음원 다운로드 완료: {title}")
            return filepath, title, thumbnail, webpage_url
        except Exception as e:
            last_error = e
            print(f"⚠️ YouTube 다운로드 재시도: {type(e).__name__}: {e}")
            for candidate in Path("/tmp").glob(f"discord_music_{guild_id}_{unique}.*"):
                _cleanup_file(str(candidate))

    raise RuntimeError(f"YouTube에서 음원을 가져오지 못했습니다: {last_error}") from last_error
