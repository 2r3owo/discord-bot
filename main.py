...
class _SilentYTDLPLogger:
    ...
    def error(self, msg):
        print(f"⚠️ yt-dlp 오류: {msg}")


def make_youtube_options(mode, outtmpl):
    """YouTube 다운로드 설정만 담당합니다.
    다른 기능은 건드리지 않고 음악 다운로드 방식만 재시도합니다.
    """
    ...
    if mode == "default":
        # yt-dlp가 현재 사용 가능한 YouTube 클라이언트를 직접 선택하도록 둡니다.
        # 강제 extractor_args를 넣지 않는 것이 최근 YouTube 변경에 더 안전합니다.
        return opts

    if mode == "nocookie_web_embedded":
        opts["extractor_args"] = {
...
def download_song(search_or_url, guild_id):
    ...
    """
    query = search_or_url if search_or_url.startswith(("https://", "http://")) else f"ytsearch1:{search_or_url}"
    ...
    # 기본 설정을 먼저 시도하고, 실패할 때만 클라이언트별 설정을 순서대로 시도합니다.
    # 특정 클라이언트를 강제로 고정하면 YouTube 정책 변경 때 전부 실패할 수 있습니다.
    modes = [
        "default",
        "nocookie_web_embedded",
        "nocookie_tv",
        "nocookie_web",
        "nocookie_android",
        "cookie_web",
    ...
    for mode in modes:
        try:
            ...
            with yt_dlp.YoutubeDL(opts) as ydl:
                # 링크와 검색어 모두 같은 실제 다운로드 경로를 사용합니다.
                # 기존의 process_info(info)는 검색 결과/직접 링크에서
                # 실제 파일을 만들지 못하는 경우가 있어 download=True로 처리합니다.
                ...
                webpage_url = info.get('webpage_url') or info.get('original_url') or (search_or_url if search_or_url.startswith(('https://', 'http://')) else '')
                print(f"⬇️ 음원 다운로드 완료 ({mode}): {title}")
    ...
    if last_error:
        print(f"❌ 모든 YouTube 다운로드 방식 실패: {type(last_error).__name__}: {last_error}")
    short_error = str(last_error).replace("\n", " ").strip() if last_error else "알 수 없는 오류"
    if len(short_error) > 700:
        short_error = short_error[:700] + "..."
    raise RuntimeError(
...
music_current_song = {}
# 정지 버튼으로 멈춘 곡: 다시 재생할 수 있도록 마지막 곡 정보를 보관합니다.
music_stopped_song = {}
...
async def update_music_status(guild: discord.Guild, channel: discord.abc.Messageable):
    """음악 플레이어 상태창을 한 개만 유지하고 내용만 갱신합니다."""
    guild_id = guild.id
    lock = music_status_locks.setdefault(guild_id, asyncio.Lock())

    # /야재생해 직후 콜백과 명령어가 동시에 실행되면서
    # 상태창이 2개 생성되는 race condition을 막습니다.
    async with lock:
        ...
        current_song_obj = music_current_song.get(guild_id) or music_stopped_song.get(guild_id)
        thumbnail = current_song_obj.get('thumbnail') if current_song_obj else None
        if thumbnail:
            try:
        ...
        try:
            ...
            # 봇 재시작 등으로 메모리에 메시지 ID가 없더라도
            # 최근 메시지에서 기존 플레이어를 찾아 재사용합니다.
            existing = []
            if hasattr(channel, 'history'):
                try:
                    async for old in channel.history(limit=20):
                        if old.author.id == bot.user.id and old.embeds:
                            if old.embeds[0].title == "🎶  하치와래 MUSIC":
                                existing.append(old)
                except Exception as e:
                    print(f"⚠️ 기존 음악 플레이어 검색 실패: {e!r}")
            ...
            if existing:
                ...
                # 이미 중복으로 생긴 플레이어가 있으면 오래된 것들은 정리합니다.
            ...
            music_status_messages.pop(guild_id, None)
            try:
                message = await channel.send(embed=embed, view=view)
                music_status_messages[guild_id] = message
            except Exception as e:
...
def _chart_embed(chart_type, tracks, page=0):
    name = "멜론 TOP100" if chart_type == "top100" else "멜론 최신곡 100"
    per_page = 20
    start = page * per_page
...
class MusicChartButtonView(discord.ui.View):
    ...
    @discord.ui.button(label="즐겨찾기", emoji="⭐", style=discord.ButtonStyle.secondary)
...
# 유튜브 플레이리스트 버튼 기능
# =====================
class YouTubePlaylistModal(discord.ui.Modal, title="유튜브 플레이리스트"):
    playlist = discord.ui.TextInput(
        label="유튜브 플레이리스트 링크 또는 검색어",
        placeholder="유튜브 플레이리스트 URL / 플리 이름",
        required=True,
        max_length=300,
    )

    async def on_submit(self, interaction: discord.Interaction):
        value = str(self.playlist).strip()
        await interaction.response.defer(ephemeral=True)
        try:
            tracks = await asyncio.to_thread(_melon_playlist_tracks, value, 50)
            if not tracks:
                await interaction.followup.send("❌ 플리에서 재생할 곡을 찾지 못했어요.", ephemeral=True)
                return
            await interaction.followup.send(
                embed=_chart_embed("latest", tracks, 0),
                view=ChartView(interaction.guild.id, "melon_playlist", tracks, 0),
                ephemeral=True,
            )
        except Exception as e:
            error_text = str(e).replace("\x00", "").strip()
            print(f"❌ 유튜브 플리 불러오기 오류: {error_text!r}")

            # Discord 메시지/Embed 길이 제한을 넘지 않도록 오류 내용을 잘라냅니다.
            if len(error_text) > 1500:
                error_text = error_text[:1500] + "..."

            await interaction.followup.send(
                f"❌ 유튜브 플리 불러오기 실패: {error_text}",
                ephemeral=True,
            )


def _melon_playlist_tracks(value, limit=50):
    """YouTube 플레이리스트 링크를 받아 곡 목록을 반환합니다.
    멜론 링크는 기존 방식으로 처리하고, YouTube playlist URL은
    yt-dlp로 플레이리스트 내부 곡을 직접 읽습니다.
    """
    value = str(value).strip()
    tracks = []

    def add_entry(entry, fallback_artist="YouTube"):
        if not entry:
            return

        title = (entry.get("title") or "").strip()
        song_url = entry.get("webpage_url") or entry.get("original_url") or entry.get("url")

        if not title or not song_url:
            return

        # 같은 곡이 중복으로 들어오는 것을 방지합니다.
        if any(item["query"] == song_url for item in tracks):
            return
    ...
    # YouTube 플레이리스트 링크를 입력한 경우
    is_youtube_playlist = (
        "youtube.com/playlist" in value
        or "youtube.com/watch" in value and "list=" in value
        or "youtu.be/" in value and "list=" in value
    )
    ...
    # 멜론 링크 처리는 사용하지 않고, 유튜브 플레이리스트 링크만 처리합니다.
    if False and value.startswith(("http://", "https://")) and "melon.com" in value:
        try:
            req = Request(
                value,
                headers={
            ...
            patterns = [
                r'<a[^>]+class=["\'][^"\']*(?:fc_gray|ellipsis)[^"\']*["\'][^>]*>(.*?)</a>',
                r'<span[^>]+class=["\'][^"\']*song_name[^"\']*["\'][^>]*>(.*?)</span>',
            ]

            titles = []
            for pattern in patterns:
                titles.extend(re.findall(pattern, text, re.S | re.I))

            for raw_title in titles:
                title = html.unescape(re.sub(r"<[^>]+>", "", raw_title))
                title = re.sub(r"\s+", " ", title).strip()

                if title and not any(track["title"] == title for track in tracks):
                    tracks.append({
                        "rank": len(tracks) + 1,
                        "title": title,
                        "artist": "Melon",
            ...
            print(f"⚠️ 멜론 플리 HTML 읽기 실패: {e!r}")

        if tracks:
            return tracks[:limit]

    # 검색어를 입력한 경우: YouTube에서 플레이리스트를 검색합니다.
    search_options = {
        "quiet": True,
        "no_warnings": True,
        "extract_flat": True,
    ...
    with yt_dlp.YoutubeDL(search_options) as ydl:
        search_info = ydl.extract_info(
            f"ytsearch10:{value} 플레이리스트",
            download=False,
        )

    # 검색 결과 중 실제 playlist URL을 우선 선택합니다.
    candidates = (search_info or {}).get("entries") or []
    for candidate in candidates:
        ...
        try:
            ...
            playlist_entries = (playlist_info or {}).get("entries") or []
            for entry in playlist_entries:
                add_entry(entry)
                if len(tracks) >= limit:
                    return tracks[:limit]
        except Exception as e:
            print(f"⚠️ YouTube 플레이리스트 검색 결과 건너뜀: {e!r}")

    return tracks[:limit]


class MelonPlaylistButtonView(discord.ui.View):
    def __init__(self):
    ...
    @discord.ui.button(
        ...
        custom_id="music_my_youtube_playlist",
        row=3,
    )
    async def my_playlist(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(YouTubePlaylistModal())

    @discord.ui.button(
        label="플리 검색",
        emoji="🔎",
        style=discord.ButtonStyle.secondary,
        custom_id="music_search_youtube_playlist",
        row=3,
    )
    async def search_playlist(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(YouTubePlaylistModal())
...
class MusicPlayerView(discord.ui.View):
    def __init__(self):
        super().__init__(timeout=None)
        youtube_buttons = MelonPlaylistButtonView()
        self.add_item(youtube_buttons.children[0])
        self.add_item(youtube_buttons.children[1])
...
async def on_ready():
    ...
    if not getattr(bot,"_music_view_started",False):
        ...
        bot.add_view(MelonPlaylistButtonView())
