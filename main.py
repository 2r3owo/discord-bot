...
def download_song(search_or_url, guild_id):
    ...
    이 함수만 수정합니다. 기존 명령어와 대기열 로직은 그대로 둡니다.
    """
    ...
    # 이전 코드의 클라이언트 강제 설정과 잘못된 PO Token 주소를 제거합니다.
    ...
    try:
        with yt_dlp.YoutubeDL(opts) as ydl:
            ...
            if not info:
                raise RuntimeError("YouTube에서 영상 정보를 가져오지 못했습니다.")
        ...
        print(f"❌ YouTube 다운로드 실패: {type(e).__name__}: {e}")
        raise RuntimeError("YouTube에서 음원을 가져오지 못했습니다.") from e
