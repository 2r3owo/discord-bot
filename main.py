...
def _download_youtube(search, guild_id):
    """
    YouTube 오디오를 /tmp에 다운로드합니다.
    한 가지 YouTube 클라이언트가 일시적으로 막혀도 다른 방식으로 재시도합니다.
    기존 쿠키는 그대로 사용하되, 쿠키가 문제인 경우에는 쿠키 없는 방식도 시도합니다.
    """
    ...
    # 순서대로 여러 방식을 시도합니다.
    # 같은 IP가 YouTube에서 완전히 차단된 경우에는 모든 방식이 실패할 수 있습니다.
    attempts = [
        ("기본 + 쿠키", {}),
        ("web_safari", {
            "extractor_args": {
    ...
    for label, extra in attempts:
        # 매 시도마다 파일 이름을 달리하지 않고 같은 output_base를 사용합니다.
        # 실패한 찌꺼기는 다음 시도 전에 정리합니다.
        for stale in __import__("glob").glob(output_base + ".*"):
            try:
                os.remove(stale)
            except Exception:
                pass
        ...
        if extra.get("remove_cookie"):
            options.pop("cookiefile", None)

        if "extractor_args" in extra:
            options["extractor_args"] = extra["extractor_args"]

        print(f"🔎 YouTube 재생 방식 시도: {label}")
        ...
        try:
            with yt_dlp.YoutubeDL(options) as ydl:
                ...
                # 확장자가 바뀌거나 postprocessor가 개입해도 찾도록 glob fallback 사용
                matches = __import__("glob").glob(output_base + ".*")
                candidates.extend(matches)

                filepath = next(
                    (fp for fp in candidates if fp and os.path.isfile(fp) and os.path.getsize(fp) > 0),
                    None
                )
                ...
                print(f"✅ YouTube 재생 준비 완료: {label} / {title}")
            ...
            print(f"⚠️ YouTube 방식 실패 ({label}): {msg}")

    # 모든 방식이 실패한 경우에는 마지막 오류만 그대로 던지지 않고
    # 원인을 알아보기 쉽게 전달합니다.
    if errors:
        last = errors[-1]
        raise RuntimeError(
            "YouTube에서 음악을 가져오지 못했습니다.\n"
            f"마지막 시도: {last}\n"
            "여러 YouTube 재생 방식을 자동으로 재시도했지만 모두 실패했습니다."
        )

    raise RuntimeError("YouTube에서 음악을 가져오지 못했습니다.")
...
async def 야재생해(interaction: discord.Interaction, search: str):
    ...
    try:
        ...
        print(f"❌ YouTube 재생 최종 오류: {e}")
        await interaction.followup.send(
            "❌ YouTube에서 음악을 가져오지 못했어요.\n"
            "잠시 후 다시 시도해 주세요.",
            ephemeral=True
        )
...
async def help_command(interaction: discord.Interaction):
    ...
    embed.add_field(
        name="🎡 결정 룰렛",
        value="`/룰렛`: 선택지를 넣고 랜덤으로 하나를 골라줍니다.\n"
              "🎡 설정 → 선택지 입력 / 🎲 돌리기 → 결과 선택",
        inline=False
    )
...
async def on_ready():
    try:
        # 중요:
        # 예전에 길드 명령어로 복사해 둔 명령어와 전역 명령어가 동시에
        # 존재하면 Discord 검색창에 모든 명령어가 2개씩 보일 수 있습니다.
        # 기존 길드 명령어를 먼저 비우고 전역 명령어만 사용하도록 합니다.
        cleared_guilds = 0
        for guild in bot.guilds:
            try:
                ...
                print(f"⚠️ {guild.name} 기존 길드 명령어 정리 실패: {guild_error}")
        ...
        print(
            f"✅ {bot.user} 연결 완료! "
            f"기존 길드 명령어 {cleared_guilds}개 서버 정리 / "
            f"전역 명령어 {len(synced_global)}개 동기화"
        )
    except Exception as e:
