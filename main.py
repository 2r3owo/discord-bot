...
def download_song(search_or_url, guild_id):
    ...
    # 쿠키가 설정되어 있으면 인증이 필요한 YouTube 요청을 먼저 시도합니다.
    # 쿠키가 없을 때만 기존 무쿠키 방식으로 진행합니다.
    if YT_COOKIE_FILE:
        modes = [
            "cookie_android",
            "cookie_web",
    ...
    for mode in modes:
        try:
            ...
            with yt_dlp.YoutubeDL(opts) as ydl:
                # 검색과 다운로드를 한 번에 수행합니다.
                # 먼저 정보를 조회한 뒤 같은 URL을 다시 요청하면 YouTube가
                # 두 번째 요청을 차단하는 경우가 있어, download=True로 한 번만 처리합니다.
