...
def make_youtube_options(mode, outtmpl):
    ...
    if mode in ("cookie_tv", "nocookie_tv"):
        opts["format"] = "bestaudio[ext=m4a]/bestaudio/best"
        opts["extractor_args"] = {
            "youtube": {
        ...
        if mode == "cookie_tv" and YT_COOKIE_FILE:
            opts["cookiefile"] = YT_COOKIE_FILE
        return opts

    if mode in ("cookie_ios", "nocookie_ios"):
        opts["format"] = "bestaudio[ext=m4a]/bestaudio/best"
        opts["extractor_args"] = {
            "youtube": {
                "player_client": ["ios"],
        ...
        if mode == "cookie_ios" and YT_COOKIE_FILE:
            opts["cookiefile"] = YT_COOKIE_FILE
        return opts
...
function start(e){e.preventDefault();h.push(state());if(h.length>30)h.shift();f=[];let p=pos(e);lx=p.x;ly=p.y;down=true;
  if(tool==='pixel'){pixel(p);c.setPointerCapture?.(e.pointerId)}
...
function move(e){if(!down)return;e.preventDefault();let p=pos(e);setup();
 ...
 if(tool==='pixel'){pixel(p);lx=p.x;ly=p.y;return}
...
function end(e){down=false;try{if(e)c.releasePointerCapture?.(e.pointerId)}catch(_){} }
