...
def make_youtube_options(mode, outtmpl):
    ...
    현재 YouTube의 클라이언트별 제한 때문에 한 방식이 실패하면
    다음 방식으로 자동 재시도합니다.

    우선순위:
      1. mweb + bgutil PO Token (쿠키 없음)  ← 공식 권장 방식
      2. web_safari (쿠키 없음, HLS 우선)
      3. web_embedded (쿠키 없음)
      4. android_vr (쿠키 없음)
      5. tv (쿠키 없음)
      6. web_creator + 기존 쿠키 + bgutil PO Token

    중요: mweb에 기존 브라우저 쿠키를 먼저 넣으면 최근 YouTube 변경으로
    "The page needs to be reloaded"가 발생할 수 있어 쿠키 없는 mweb을 우선합니다.
    "Sign in to confirm you're not a bot"이 나오면 다음 클라이언트로 넘어갑니다.
    """
    ...
    if mode == 'web_safari':
        ...
        # web_safari의 HLS(m3u8)를 우선 사용하도록 요청
        opts['format'] = 'bestaudio[protocol=m3u8_native]/bestaudio/best'
        return opts
    ...
    if mode == 'android_vr_18':
        opts['extractor_args'] = {
            'youtube': {
                'player_client': ['android_vr'],
            },
        }
        # 최근 yt-dlp/YouTube 동작상 android_vr의 일반 https 포맷은
        # GVS PO Token을 요구할 수 있지만 format 18은 별도 예외로
        # 남아 있는 경우가 있습니다. format 18에는 AAC 오디오가
        # 포함되어 있으므로 FFmpeg에서 음원만 추출할 수 있습니다.
    ...
    if mode == 'web_creator_cookie':
        ...
        if YT_COOKIE_FILE:
            ...
            raise RuntimeError('web_creator에 사용할 YouTube 쿠키가 없습니다.')
        return opts
