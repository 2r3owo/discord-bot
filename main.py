...
def download_song(search_or_url, guild_id):
    ...
    # YouTube가 계정 쿠키를 넣은 요청을 오히려 봇 인증으로 막는 경우가 있어
    # 쿠키 없는 클라이언트를 먼저 시도하고, 마지막에만 쿠키를 사용합니다.
    # 만료된 쿠키를 반복 사용하면 YouTube가 모든 요청을 봇 인증으로 막습니다.
    # 따라서 쿠키 방식은 사용하지 않고, 지원되는 클라이언트만 순서대로 시도합니다.
    modes = [
        "nocookie_android",
        "nocookie_web",
        "nocookie_ios",
    ...
    for mode in modes:
        try:
            ...
            # YouTube의 긴 인증 오류가 콘솔/디스코드에 그대로 노출되지 않도록 합니다.
            opts.update({
                "quiet": True,
                "no_warnings": True,
                "noprogress": True,
            ...
            with yt_dlp.YoutubeDL(opts) as ydl:
                # 검색/링크 정보를 먼저 확인한 뒤, yt-dlp의 정식 다운로드 경로로
                # 음원을 내려받습니다. process_info(info)는 최근 YouTube 응답에서
                # 실제 다운로드가 누락되는 경우가 있어 download=True를 사용합니다.
                ...
                # 검색 결과의 실제 영상 URL을 다시 다운로드합니다.
                download_url = info.get('webpage_url') or info.get('original_url') or query
                downloaded_info = ydl.extract_info(download_url, download=True)
                if downloaded_info:
    ...
    raise RuntimeError(
        "YouTube에서 음원을 가져오지 못했습니다. 잠시 후 다시 시도하거나 다른 영상을 사용해 주세요."
    )
...
class MelonPlaylistModal(discord.ui.Modal, title="멜론 플레이리스트 재생"):
    playlist = discord.ui.TextInput(label="멜론 플리 링크 또는 플리 번호", placeholder="https://www.melon.com/playlist/detail/view.htm?plId=123456", required=True)
    async def on_submit(self, interaction: discord.Interaction):
        value = str(self.playlist.value).strip()
        m = re.search(r"(?:plId=|playlist/)(\d+)", value)
        if not m and value.isdigit(): m = re.match(r"(\d+)", value)
        if not m:
            return await interaction.response.send_message("❌ 멜론 플레이리스트 링크나 번호를 입력해 주세요.", ephemeral=True)
        await _play_playlist(interaction, m.group(1), "내 멜론 플레이리스트")
