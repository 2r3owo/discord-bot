...
class MelonPlaylistModal(discord.ui.Modal, title="멜론 플레이리스트"):
    playlist = discord.ui.TextInput(label="멜론 플리 링크 또는 검색어", placeholder="멜론 플레이리스트 URL / 플리 이름", required=True, max_length=300)

    async def on_submit(self, interaction: discord.Interaction):
        value = str(self.playlist).strip()
        await interaction.response.defer(ephemeral=True)
        try:
            # 멜론 플리 URL은 yt-dlp가 직접 읽지 못할 수 있어 검색어로도 안전하게 처리합니다.
            if value.startswith(('http://', 'https://')):
                query = value
            else:
                query = f"멜론 플레이리스트 {value}"
            tracks = await asyncio.to_thread(_melon_playlist_tracks, query, 50)
            if not tracks:
                return await interaction.followup.send("❌ 플리에서 재생할 곡을 찾지 못했어요.", ephemeral=True)
            await interaction.followup.send(
            ...
            await interaction.followup.send(f"❌ 멜론 플리 불러오기 실패: {e}", ephemeral=True)


def _melon_playlist_tracks(value, limit=50):
    # 멜론 링크가 제공되면 페이지의 곡 제목을 추출하고, 실패하면 검색 결과로 대체합니다.
    tracks = []
    if value.startswith(('http://', 'https://')):
        try:
            req = Request(value, headers={'User-Agent': 'Mozilla/5.0'})
            text = urlopen(req, timeout=20).read().decode('utf-8', errors='ignore')
            titles = re.findall(r'<a[^>]+class="[^" ]*(?:fc_gray|ellipsis)[^" ]*"[^>]*>(.*?)</a>', text, re.S)
            titles += re.findall(r'<span[^>]+class="[^" ]*song_name[^" ]*"[^>]*>(.*?)</span>', text, re.S)
            for raw in titles:
                title = re.sub(r'<[^>]+>', '', raw).strip()
                title = html.unescape(re.sub(r'\s+', ' ', title))
                if title and not any(t['title'] == title for t in tracks):
                    tracks.append({'title': title, 'artist': 'Melon', 'query': title, 'thumbnail': ''})
                if len(tracks) >= limit: break
        except Exception:
            pass
    if not tracks:
        with yt_dlp.YoutubeDL({'quiet': True, 'extract_flat': True, 'skip_download': True, 'default_search': 'ytsearch'}) as ydl:
            info = ydl.extract_info(f"ytsearch{limit}:{value}", download=False)
            for e in (info or {}).get('entries', [])[:limit]:
                if e:
                    tracks.append({'title': e.get('title','알 수 없는 곡'), 'artist': 'YouTube', 'query': e.get('webpage_url') or e.get('url') or e.get('title',''), 'thumbnail': e.get('thumbnail','')})
    return tracks
...
class MelonPlaylistButtonView(discord.ui.View):
    ...
    @discord.ui.button(label="내 멜론 플리", emoji="🎵", style=discord.ButtonStyle.primary, custom_id="music_my_melon_playlist", row=3)
    async def my_playlist(self, interaction, button):
        await interaction.response.send_modal(MelonPlaylistModal())

    @discord.ui.button(label="플리 검색", emoji="🔎", style=discord.ButtonStyle.secondary, custom_id="music_search_melon_playlist", row=3)
...
class MusicPlayerView(discord.ui.View):
    def __init__(self):
        super().__init__(timeout=None)
        self.add_item(MelonPlaylistButtonView().children[0])
        self.add_item(MelonPlaylistButtonView().children[1])
