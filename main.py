...
YTDLP_QUIET_LOGGER = _QuietYTDLPLogger()
...
async def 야드루와(interaction: discord.Interaction):
    ...
    try:
        ...
        await interaction.response.send_message("❌ 음성 채널에 연결하지 못했어요. 잠시 후 다시 시도해 주세요.", ephemeral=True)
...
async def 야재생해(interaction: discord.Interaction, search: str):
    ...
    try:
        ...
        await interaction.followup.send("❌ 음악을 가져오지 못했어요. 잠시 후 다시 시도해 주세요.", ephemeral=True)
...
async def 야기다려(interaction: discord.Interaction, search: str):
    ...
    try:
        ...
        await interaction.followup.send("❌ 음악을 대기열에 추가하지 못했어요. 잠시 후 다시 시도해 주세요.", ephemeral=True)
...
# 닉네임 변경 기능만 추가
# =====================
@bot.tree.command(name="닉네임변경", description="본인 닉네임을 변경합니다.")
async def 닉네임변경(interaction: discord.Interaction, 새닉네임: str):
    if interaction.guild is None:
        return await interaction.response.send_message("❌ 서버에서 사용해 주세요.", ephemeral=True)
    새닉네임 = 새닉네임.strip()
    if not 새닉네임:
        return await interaction.response.send_message("❌ 닉네임을 입력해 주세요.", ephemeral=True)
    if len(새닉네임) > 32:
        return await interaction.response.send_message("❌ 닉네임은 32자 이하로 입력해 주세요.", ephemeral=True)
    try:
        await interaction.user.edit(nick=새닉네임)
        await interaction.response.send_message(f"✅ 닉네임을 **{discord.utils.escape_markdown(새닉네임)}**(으)로 변경했어요!", ephemeral=True)
    except discord.Forbidden:
        await interaction.response.send_message("❌ 봇에게 닉네임 변경 권한이 없어요.", ephemeral=True)
    except Exception as e:
        print("닉네임 변경 오류:", e)
