...
def _melon_playlist_tracks(value, limit=50):
    """멜론 플리 URL의 곡명을 읽고, 실패하면 YouTube 검색으로 대체합니다."""
    tracks = []
    ...
    if not tracks:
        ...
        with yt_dlp.YoutubeDL(options) as ydl:
            info = ydl.extract_info(f"ytsearch{limit}:{search_text}", download=False)
            for entry in (info or {}).get("entries", [])[:limit]:
                if not entry:
                    continue
                tracks.append({
                    "rank": len(tracks) + 1,
                    "title": entry.get("title") or "알 수 없는 곡",
                    "artist": "YouTube",
                    "query": entry.get("webpage_url") or entry.get("url") or entry.get("title", ""),
