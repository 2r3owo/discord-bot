...
# Railway에서 YouTube 접속이 차단될 때 사용할 프록시입니다.
# 환경변수 YOUTUBE_PROXY에 프록시 주소를 넣으면 일반 재생과 차트에 모두 적용됩니다.
...
def handle_song_finished(guild: discord.Guild, channel: discord.abc.Messageable, finished_song):
    ...
    if queues.get(guild_id):
        next_song = queues[guild_id].popleft()
        try:
            if next_song.get("needs_download") or not next_song.get("filepath"):
                filepath, title, thumbnail, webpage_url = download_song(next_song.get("query", ""), guild_id)
                next_song.update({"filepath": filepath, "title": title, "thumbnail": thumbnail or next_song.get("thumbnail", ""), "webpage_url": webpage_url or next_song.get("webpage_url", ""), "needs_download": False})
...
class MusicPlayerView(discord.ui.View):
    ...
    @discord.ui.button(label="🏆", style=discord.ButtonStyle.secondary, custom_id="music_youtube_top100", row=2)
    async def youtube_top100(self, interaction: discord.Interaction, button: discord.ui.Button):
        await start_youtube_chart(interaction, "top100")

    @discord.ui.button(label="NEW", style=discord.ButtonStyle.success, custom_id="music_youtube_latest", row=2)
    async def youtube_latest(self, interaction: discord.Interaction, button: discord.ui.Button):
        await start_youtube_chart(interaction, "latest")
...
function start(e){
  ...
  else if(tool==='pixel' || $('pixelMode').checked){drawPixel(p.x,p.y);down=false;}
}
function move(e){
  if(!down)return;
  e.preventDefault();
...
def _fetch_youtube_chart(chart_type="top100", limit=100):
    """YouTube 차트 목록을 가져옵니다.

    차트 조회도 일반 음악 재생과 동일하게 프록시/쿠키/클라이언트 fallback을
    사용해야 Railway에서 차트만 따로 봇 차단되는 문제가 생기지 않습니다.
    """
    ...
    modes = ["cookie_android", "cookie_web", "cookie_mweb", "nocookie_android", "nocookie_web", "nocookie_mweb"]
    last_error = None

    for mode in modes:
        for target in sources:
            try:
                opts = make_youtube_options(mode, "/tmp/unused_chart_%(id)s.%(ext)s")
                opts.update({
                    "quiet": True,
                    "no_warnings": True,
                    "skip_download": True,
                ...
                # 차트 목록 조회에서는 실제 다운로드를 하지 않으므로
                # 검색 결과의 URL을 그대로 대기열에 보관합니다.
                with yt_dlp.YoutubeDL(opts) as ydl:
                ...
                for entry in entries:
                    ...
                    if len(result) >= limit:
                        break
                if result:
                    print(f"✅ YouTube 차트 목록 확보: {mode}, {len(result)}곡")
                    return result[:limit]
            except Exception as exc:
                last_error = exc
                print(f"⚠️ YouTube 차트 소스 실패 ({mode}): {target} / {str(exc)[:300]}")
...
async def start_youtube_chart(interaction: discord.Interaction, chart_type="top100"):
    ...
    try:
        ...
        tracks = await asyncio.to_thread(_fetch_youtube_chart, chart_type, 100)
        if guild.voice_client.is_playing() or guild.voice_client.is_paused():
            guild.voice_client.stop()
        queues[guild.id] = deque()
        music_history[guild.id] = []
        music_stopped_song.pop(guild.id, None)

        # 검색 결과는 URL만 먼저 대기열에 넣고, 첫 곡부터 순서대로 다운로드합니다.
        for track in tracks:
        ...
        first = queues[guild.id].popleft()
        filepath, title, thumbnail, webpage_url = await asyncio.to_thread(download_song, first["query"], guild.id)
        first.update({"filepath": filepath, "title": title, "thumbnail": thumbnail or first.get("thumbnail", ""), "webpage_url": webpage_url or first["query"], "needs_download": False})
        _play_song_now(guild, first, interaction.channel)
        await update_music_status(guild, interaction.channel)
        name = "YouTube 인기 음악 100" if chart_type == "top100" else "YouTube 최신곡 100"
        await interaction.followup.send(f"✅ **{name}**을 불러왔어요. 총 {len(tracks)}곡을 순서대로 재생합니다.", ephemeral=True)
    except Exception as e:
        print(f"❌ YouTube 차트 자동재생 실패: {e!r}")
        await interaction.followup.send(f"❌ YouTube 차트를 불러오지 못했어요: {e}", ephemeral=True)
