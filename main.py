...
# 멜론 플레이리스트 버튼 기능
# =====================
class MelonPlaylistModal(discord.ui.Modal, title="멜론 플레이리스트"):
    playlist = discord.ui.TextInput(
        label="멜론 플리 링크 또는 검색어",
        placeholder="멜론 플레이리스트 URL / 플리 이름",
        required=True,
        max_length=300,
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            print(f"❌ 멜론 플리 불러오기 오류: {error_text!r}")
...
def _melon_playlist_tracks(value, limit=50):
    ...
    # 멜론 링크는 기존 HTML 추출 방식을 유지합니다.
    if value.startswith(("http://", "https://")) and "melon.com" in value:
        try:
            req = Request(
                value,
                headers={
...
class MelonPlaylistButtonView(discord.ui.View):
    ...
    @discord.ui.button(
        ...
        custom_id="music_my_melon_playlist",
        row=3,
    )
    ...
    @discord.ui.button(
        ...
        custom_id="music_search_melon_playlist",
        row=3,
    )
...
class MusicPlayerView(discord.ui.View):
    def __init__(self):
        super().__init__(timeout=None)
        melon_buttons = MelonPlaylistButtonView()
        self.add_item(melon_buttons.children[0])
        self.add_item(melon_buttons.children[1])
