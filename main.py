...
def make_youtube_options(mode, outtmpl):
    ...
    opts["extractor_args"] = {
        "youtube": {
            "player_client": ["web_safari", "web_embedded", "tv", "android", "ios"],
        }
    }
...
def download_song(search_or_url, guild_id):
    ...
    # YouTube가 최근 일부 클라이언트의 음원 요청을 봇으로 판정하므로
    # 실패율이 높은 조합은 빼고, 검색과 실제 다운로드를 분리해서 시도합니다.
    modes = [
        "nocookie_android",
        "nocookie_ios",
        "nocookie_tv",
    ...
    for mode in modes:
        try:
            ...
            with yt_dlp.YoutubeDL(opts) as ydl:
                # 검색 결과를 먼저 얻고, 실제 영상 URL로 다시 추출합니다.
                # 검색 객체를 바로 process_info()에 넘기면 최신 yt-dlp에서
                # 파일명이 꼬이거나 YouTube 인증 오류가 반복될 수 있습니다.
                ...
                if not video_url:
                    raise RuntimeError("YouTube 영상 주소를 받지 못했습니다.")
                ...
                # extract_info(download=False)만 호출하면 실제 파일이 만들어지지
                # 않을 수 있으므로, 영상 URL을 명시적으로 다운로드합니다.
