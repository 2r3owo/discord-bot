...
def download_song(search_or_url, guild_id):
    ...
    # 쿠키가 들어간 클라이언트는 YouTube의 bot/challenge 검사를
    # 더 자주 유발할 수 있어 쿠키 없는 클라이언트를 먼저 시도합니다.
    modes = [
        "nocookie_web_embedded",
        "nocookie_tv_embedded",
        "nocookie_ios",
        "nocookie_android",
        "nocookie_web",
        "nocookie_mweb",
    ...
    # Railway에 저장된 YouTube 쿠키가 만료되거나 회전되면
    # "Sign in to confirm you're not a bot" 오류가 반복됩니다.
    # 기본값은 쿠키 재시도를 사용하지 않도록 하며,
    # 정말 유효한 쿠키를 새로 넣었을 때만 YOUTUBE_USE_COOKIES=1로 활성화합니다.
    if os.getenv("YOUTUBE_USE_COOKIES", "0").strip() == "1" and YT_COOKIE_FILE:
        modes.extend(["cookie_android", "cookie_web", "cookie_mweb"])
