...
def _fetch_melon_chart(chart_type="top100", limit=100):
    url = "https://www.melon.com/chart/index.htm" if chart_type == "top100" else "https://www.melon.com/new/index.htm"
    req = Request(
        url,
        headers={
            "User-Agent": (
    ...
    rows = re.findall(r'<tr[^>]*class="[^"]*lst50[^"]*"[^>]*>(.*?)</tr>', raw, re.S)
    if not rows:
        rows = re.findall(r'<tr[^>]*class="[^"]*lst100[^"]*"[^>]*>(.*?)</tr>', raw, re.S)

    result = []
    for row in rows[:limit]:
        title = re.search(
            r'<div[^>]*class="[^"]*ellipsis[^"]*rank01[^"]*"[^>]*>.*?<a[^>]*>(.*?)</a>',
            row, re.S
        )
        artist = re.search(
            r'<div[^>]*class="[^"]*ellipsis[^"]*rank02[^"]*"[^>]*>.*?<a[^>]*>(.*?)</a>',
            row, re.S
        )
        image = re.search(r'<img[^>]*src="([^"]+)"', row, re.S)
        if not title or not artist:
            continue
        result.append({
            "rank": len(result) + 1,
            "title": _clean_chart_text(title.group(1)),
            "artist": _clean_chart_text(artist.group(1)),
            "query": f"{_clean_chart_text(title.group(1))} {_clean_chart_text(artist.group(1))}",
            "thumbnail": image.group(1) if image else "",
        })
    if not result:
        raise RuntimeError("멜론 차트 정보를 가져오지 못했어요.")
    return result
...
class MusicChartButtonView(discord.ui.View):
    ...
    @discord.ui.button(label="멜론 TOP100", emoji="🏆", style=discord.ButtonStyle.secondary)
    async def top100(self, interaction, button):
        await self._show(interaction, "top100")

    @discord.ui.button(label="최신곡 100", emoji="🆕", style=discord.ButtonStyle.secondary)
    async def latest(self, interaction, button):
        await self._show(interaction, "latest")
...
class MusicPlayerView(discord.ui.View):
    ...
    @discord.ui.button(label="", emoji="🏆", style=discord.ButtonStyle.secondary, custom_id="music_top100", row=2)
    async def top100_chart(self, interaction: discord.Interaction, button: discord.ui.Button):
        await MusicChartButtonView(interaction.guild.id)._show(interaction, "top100")

    @discord.ui.button(label="", emoji="🆕", style=discord.ButtonStyle.secondary, custom_id="music_latest100", row=2)
    async def latest_chart(self, interaction: discord.Interaction, button: discord.ui.Button):
        await MusicChartButtonView(interaction.guild.id)._show(interaction, "latest")
