...
async def on_ready():
    try:
        # 전역 명령어는 Discord 반영까지 시간이 걸릴 수 있으므로,
        # 봇이 들어가 있는 서버에는 길드 명령어로 즉시 동기화합니다.
        synced_global = await bot.tree.sync()

        synced_guild_total = 0
        for guild in bot.guilds:
            try:
                bot.tree.copy_global_to(guild=guild)
                synced_guild = await bot.tree.sync(guild=guild)
                synced_guild_total += len(synced_guild)
            except Exception as guild_error:
                print(f"⚠️ {guild.name} 길드 명령어 동기화 실패: {guild_error}")

        print(
            f"✅ {bot.user} 연결 완료! "
            f"전역 {len(synced_global)}개 / 서버 즉시 동기화 {synced_guild_total}개"
        )
    except Exception as e:
