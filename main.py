...
# =====================
# 보조 함수 (대기열 관리) - 음악 부분만 수정
# =====================
def _music_query(value: str) -> str:
    return value if value.startswith(("http://", "https://")) else f"ytsearch1:{value}"


def _download_music_sync(query: str, guild_id: int):
    """YouTube 주소를 FFmpeg에 직접 넘기지 않고 로컬 파일로 내려받습니다."""
    base = f"/tmp/discord_music_{guild_id}_{uuid.uuid4().hex}"
    modes = [
        {"name": "mweb_cookie", "extractor_args": {"youtube": {"player_client": ["mweb"]}}},
        {"name": "web_safari_cookie", "extractor_args": {"youtube": {"player_client": ["web_safari"]}}},
        {"name": "android_vr_cookie", "extractor_args": {"youtube": {"player_client": ["android_vr"]}}},
        {"name": "web_embedded_cookie", "extractor_args": {"youtube": {"player_client": ["web_embedded"]}}},
    ]
    last_error = None
    for mode in modes:
        try:
            opts = dict(YDL_OPTIONS)
            # 쿠키를 제거하지 않습니다. Railway에서는 쿠키가 인증에 필요할 수 있습니다.
            opts["format"] = "bestaudio[ext=m4a]/bestaudio/best"
            opts["outtmpl"] = base + ".%(ext)s"
            opts["noplaylist"] = True
            opts["quiet"] = True
            opts["no_warnings"] = True
            opts["extractor_args"] = mode["extractor_args"]
            # YouTube가 브라우저 요청처럼 인식하도록 기본 헤더를 보강합니다.
            headers = dict(opts.get("http_headers", {}))
            headers.setdefault("Accept-Language", "ko-KR,ko;q=0.9,en-US;q=0.8,en;q=0.7")
            opts["http_headers"] = headers
            opts["postprocessors"] = []
            with yt_dlp.YoutubeDL(opts) as ydl:
                info = ydl.extract_info(_music_query(query), download=True)
                if info and info.get("entries"):
                    info = info["entries"][0]
                title = (info or {}).get("title") or query
            files = [x for x in glob.glob(base + ".*") if not x.endswith(".part")]
            if not files:
                raise RuntimeError("다운로드된 음악 파일을 찾지 못했습니다.")
            return {"title": title, "file": files[0]}
        except Exception as e:
            last_error = e
            print(f"⚠️ YouTube 방식 {mode['name']} 실패: {e}")
    raise RuntimeError(f"YouTube에서 음원을 가져오지 못했습니다. 마지막 오류: {last_error}")


async def _get_music(query: str, guild_id: int):
    loop = asyncio.get_running_loop()
    return await loop.run_in_executor(None, _download_music_sync, query, guild_id)


def _play_local_song(interaction: discord.Interaction, song: dict):
    voice = interaction.guild.voice_client
    source = discord.FFmpegPCMAudio(song["file"], executable="ffmpeg", options="-vn")
    voice.play(source, after=lambda e: check_queue(interaction))


def check_queue(interaction: discord.Interaction):
    """노래가 끝나면 대기열의 다음 로컬 파일을 재생합니다."""
    guild_id = interaction.guild.id
    if guild_id in queues and queues[guild_id]:
        next_song = queues[guild_id].popleft()
        try:
            _play_local_song(interaction, next_song)
            coro = interaction.channel.send(f"🎶 다음 곡 재생: **{next_song['title']}**")
            asyncio.run_coroutine_threadsafe(coro, bot.loop)
        except Exception as e:
            print(f"⚠️ 다음 곡 재생 실패: {e}")
    else:
...
async def 야재생해(interaction: discord.Interaction, search: str):
    ...
    try:
        ...
        song = await _get_music(search, interaction.guild.id)
        if interaction.guild.voice_client.is_playing():
            interaction.guild.voice_client.stop()
        _play_local_song(interaction, song)
        await interaction.followup.send(f"🎶 즉시 재생 시작: **{song['title']}**")
    except Exception as e:
